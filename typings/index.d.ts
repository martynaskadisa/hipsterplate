...
declare global {
  ...
  /**
   * Shows if Sentry client side is enabled
   */
  const __IS_SENTRY_CLIENT_ENABLED__: boolean;

  /**
   * Sentry client side DSN
   */
  const __SENTRY_CLIENT_DSN__: string;

  /**
   * Enable redux console logger
   */
  const __IS_REDUX_LOGGING_MIDDLEWARE_ENABLED__: boolean;
